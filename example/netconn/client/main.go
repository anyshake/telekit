...
func main() {
	...
	clientID := flag.String("client-id", "netconn-client", "PSK client identity")
	...
	log.Printf("limits: frame=%d receive=%d send=%d ice=%d/%d mqtt=%d/%d",
		*maxFrameBytes, *receiveBufferBytes, *sendBufferBytes, *maxPendingICE, *maxPendingICEBytes,
		*mqttQueueMessages, *mqttQueueBytes)
	adapter, err := connectMQTT(*broker, *mqttBaseTopic, "netconn-client", *mqttQueueMessages, *mqttQueueBytes)
	if err != nil {
		log.Fatal(err)
	}
	...
	conn, err := client.NewClient(
		peer.PreSharedKey{ClientID: *clientID, Key: key[:], ServerPublicKey: serverPublicKey},
		a,
		&client.Options{
			Timeout:            *timeout,
